...
MEAN1 = np.array([1, 2])  # Mean of Gaussian distribution for Class 1
COV1 = np.array([[1, 0], [0, 1]])  # Covariance matrix for Class 1
MEAN2 = np.array([1, -2])  # Mean of Gaussian distribution for Class 2
COV2 = np.array([[1, 0], [0, 1]])  # Covariance matrix for Class 2

# Generate random points from the two components
X1 = multivariate_normal.rvs(MEAN1, COV1, CLASS1_SIZE)  # Samples for Class 1
X2 = multivariate_normal.rvs(MEAN2, COV2, CLASS2_SIZE)  # Samples for Class 2

# Combine the points and generate labels
X = np.vstack((X1, X2))  # Combine samples into a single array
...
u_node = Linear(x_node, w_node, w0_node)  # Linear transformation node
